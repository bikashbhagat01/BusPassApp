...
//    sqlQuery = "update bus set " + field + "= " + newValue + " where busid = " + busId;
    sqlQuery = QueryBuilder.getInstance()
                            .Update()
                            .FromTable("bus")
  ...
  public static boolean displayAvailableBusTimingsAndRoutes() throws SQLException, ClassNotFoundException {
    ...
    ResultSet resultSet = QueryExecutor
                                .getInstance()
                                .getResultSet(sqlQuery);

    if(!resultSet.next()){
      return false;
    }

    if (!displayDetailsFromResultSet(resultSet)) {
  ...
  public static boolean displayAvailableBusTimingsAndRoutes(int[] routeIds, int timing)
    ...
    public static boolean displayDetailsFromResultSet(ResultSet resultSet) throws SQLException {
    if(!resultSet.next()) {
      return false;
    }
    ...
    do {
      ...
      currentStopName = resultSet.getString(3);
  ...
  public static boolean clearRouteAssignments(int routeId) throws SQLException, ClassNotFoundException {
    ...
    ResultSet resultSetOfBus = QueryExecutor
                                      .getInstance()
                                      .getResultSet(sqlQuery);
